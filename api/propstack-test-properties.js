  ...
  try {
    ...
    if (id) {
      ...
      if (String(id)==='3528391') {
        const audit=object=>Object.entries(object||{}).filter(([name,raw])=>/hausgeld|wohngeld|house.?money|condominium/i.test(name)||/hausgeld|wohngeld/i.test(String(raw?.label||''))).map(([name,raw])=>{
          const value=raw && typeof raw==='object' && 'value' in raw ? raw.value : raw;
          return {name,label:/hausgeld|wohngeld/i.test(String(raw?.label||''))?raw.label:undefined,amount:typeof value==='number'&&Number.isFinite(value)?value:typeof value==='string'&&/^\d+(?:[.,]\d{1,2})?$/.test(value.trim())?value.trim():null};
        });
        console.info('Propstack house fee audit 3528391',JSON.stringify({list:audit(summary),detail:audit(detail)}));
      }

(() => {
  ...
  function renderDetail(p){
    ...
    const tour=p.tour?`<section class="pp-panel pp-tour"><h2>Virtueller Rundgang</h2><p>Entdecken Sie die Immobilie Raum für Raum.</p>${p.tourEmbed?'<div class="pp-tour-placeholder"><button type="button" class="pp-button pp-tour-load">Rundgang hier laden</button><small>Der virtuelle Rundgang wird nach dem Klick von tour.sls.de geladen.</small></div>':''}<a class="pp-button" href="${esc(p.tour)}" target="_blank" rel="noopener noreferrer">Rundgang öffnen ↗</a></section>`:'';

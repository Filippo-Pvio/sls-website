(() => {
  ...
  function renderDetail(p){
    ...
    const property=[fact('Standort',[p.zip,p.city].filter(Boolean).join(' ')),fact('Objekttyp',p.type),fact('Wohnfläche',p.area==null?null:area(p.area)),fact('Zimmer',p.rooms),fact('Schlafzimmer',p.bedrooms),fact('Badezimmer',p.baths),fact('Grundstücksfläche',p.plot==null?null:area(p.plot)),fact('Etage',pf.floor),fact('Balkone',pf.balconies),fact('Stellplatz',pf.parking),fact('Kaufpreis',format(p.price)),fact('Käuferprovision',p.courtage),fact('Bezugsfrei ab',pf.freeFrom),fact('Baujahr',p.year),fact('Letzte Modernisierung',pf.lastRenovation),fact('Zustand',pf.condition)].join('');
    const energyFacts=[fact('Energieausweis',energy.availability),fact('Energieausweistyp',energy.kind),fact('Endenergiekennwert',energy.value==null?null:`${new Intl.NumberFormat('de-DE',{maximumFractionDigits:2}).format(energy.value)} kWh/(m²·a)`),fact('Wesentlicher Energieträger',energy.fuel),fact('Baujahr',energy.buildingYear),fact('Energieeffizienzklasse',energy.rating),fact('Ausgestellt am',date(energy.issuedOn)),fact('Gültig bis',date(energy.validUntil)),fact('Baujahr Anlagentechnik',energy.equipmentYear),fact('Heizungsart',energy.heating)].join('');

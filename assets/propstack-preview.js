(() => {
  ...
  async function load(){
    try {
      ...
      else {const response=await fetch(`/api/propstack-test-properties${id?`?id=${encodeURIComponent(id)}`:''}`,{cache:'no-store'});const data=await response.json();if(!response.ok)throw new Error(data.error||'Daten nicht abrufbar');all=data.items;$('#pp-banner').textContent=id?'Getrennter Vercel-Test: Objektanfragen werden hier noch nicht versendet. Der Bewertungsrechner weiter unten ist live und kann echte Anfragen auslösen.':'Getrennter Vercel-Test mit freigegebenen Propstack-Objekten. Objektanfragen werden hier noch nicht versendet.'}

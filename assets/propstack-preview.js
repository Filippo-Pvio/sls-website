(() => {
  ...
  function renderDetail(p){
    ...
    const gallery=pictures.length?`<div class="pp-gallery"><div class="pp-gallery-stage${pictures[0].isFloorplan?' is-floorplan':''}"><img class="pp-gallery-current" src="${esc(pictures[0].src)}" alt="${esc(pictures[0].alt)}"><span class="pp-gallery-count" aria-live="polite">1 / ${pictures.length}</span><span class="pp-gallery-plan-label" ${pictures[0].isFloorplan?'':'hidden'}>Grundriss</span><button class="pp-gallery-plan-open" type="button" ${pictures[0].isFloorplan?'':'hidden'}>Vergrößern</button>${pictures.length>1?'<button class="pp-gallery-arrow pp-gallery-prev" type="button" aria-label="Vorheriges Bild">‹</button><button class="pp-gallery-arrow pp-gallery-next" type="button" aria-label="Nächstes Bild">›</button>':''}</div>${pictures.length>1?`<div class="pp-gallery-rail" aria-label="Weitere Objektbilder">${pictures.map((item,i)=>`<button type="button" class="pp-gallery-thumb${i===0?' is-active':''}${item.isFloorplan?' is-floorplan':''}" data-photo="${i}" aria-label="${item.isFloorplan?`${esc(item.alt)} – Grundriss`:`Bild ${i+1} von ${pictures.length}`} anzeigen" aria-pressed="${i===0}">${photo(item.src,item.alt)}${item.isFloorplan?'<span class="pp-thumb-plan-label">Grundriss</span>':''}</button>`).join('')}</div>`:''}</div>`:'';
    ...
    const tour=p.tour?`<section class="pp-panel pp-tour"><h2>Virtueller Rundgang</h2><p>Entdecken Sie die Immobilie Raum für Raum.</p>${p.tourEmbed?`<iframe class="pp-tour-frame" src="${esc(p.tour)}" title="Virtueller Rundgang: ${esc(p.title)}" loading="lazy" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>`:`<div class="pp-tour-placeholder"><a class="pp-button" href="${esc(p.tour)}" target="_blank" rel="noopener noreferrer">Rundgang öffnen ↗</a></div>`}</section>`:'';
    ...
    narrow.addEventListener('change',scheduleShortcut);
    scheduleShortcut();
